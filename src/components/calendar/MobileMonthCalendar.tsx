    ...
    <section className="lg:hidden space-y-4">
      <div className="grid grid-cols-2 gap-3">
        ...
        <Link
          href="/owner-request"
          scroll={false}
          className="rounded-3xl bg-[#062A55] p-4 text-white shadow-sm"
        >
          <p className="text-xs uppercase font-bold text-white/70">
            Owner
          </p>
          <p className="text-xl font-bold mt-1">Request</p>
            ...
            return (
              <div
                ...
                className={`min-h-[76px] rounded-xl border p-1 ${
                  validDay ? "bg-white border-slate-200" : "bg-slate-50 border-slate-100"
                }`}
              >
                <p
                ...
                <div className="space-y-1">
                  {dayFlights.map((flight) => (
                    <button
                      ...
                      className={`w-full rounded-lg px-1 py-1 text-left text-[10px] font-bold leading-tight text-white ${getAircraftClasses(
                        flight.aircraft.color
                      )} ${
                        selectedFlightId === flight.id ? "ring-2 ring-amber-300" : ""
                      }`}
                    >
                      <span className="block truncate">{flight.departureTime}</span>
                      <span className="block truncate">
                        {flight.aircraft.tail} {flight.ownerOnboard ? "★" : ""}
                      </span>
                    </button>
                  ))}
                  ...
                  {dayHolds.slice(0, 1).map((hold) => (
                    <div
                      key={hold.id}
                      className={`rounded-lg border px-1 py-1 text-[9px] font-bold leading-tight ${getHoldClasses(
                        hold.type
                      )}`}
                    >
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 rounded-2xl bg-slate-50 p-4">
            ...
            <p className="mt-1 text-xl font-bold text-slate-900">
              {expandedFlight.route.from} → {expandedFlight.route.to}
            </p>
          </div>
