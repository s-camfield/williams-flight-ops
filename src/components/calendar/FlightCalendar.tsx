  ...
  return (
    <section className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Always-visible aircraft schedule
          </p>
        </div>
      ...
      <div className="grid grid-cols-7 border border-slate-200 rounded-2xl overflow-hidden">
        {Array.from({ length: 35 }).map((_, i) => {
          ...
          return (
            <div
              key={i}
              className="min-h-[130px] border border-slate-100 p-3 relative bg-white"
            >
              <div
                className={
                  day < 1 || day > 31
              ...
              <div className="mt-3 space-y-2">
                ...
                {dayHolds.map((hold) => (
                  <div
                    key={hold.id}
                    className={`rounded-xl border p-3 text-sm ${getHoldClasses(
                      hold.type
                    )}`}
                  >
                    <p className="font-bold leading-tight">{hold.title}</p>
                    {hold.subtitle && (
                      <p className="text-xs mt-1">{hold.subtitle}</p>
                    )}
                  </div>
                ))}
              </div>
      ...
      <div className="grid grid-cols-5 gap-3 mt-5 rounded-2xl border border-slate-200 p-4 text-center">
        <Stat icon="✈" number="4" label="Total Flights" />
        <Stat icon="★" number="3" label="Owner (GGW)" />
        <Stat icon="🔧" number="1" label="Maintenance" />
    ...
    <div className="flex items-center gap-2">
      <span
        className={`h-4 w-4 rounded ${
          border ? "border border-dashed border-slate-400" : color
        }`}
      />
      {label}
    </div>
