      ...
      <section className="hidden lg:block bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
          ...
          <div className="flex gap-2">
            <button className="rounded-xl border border-slate-300 px-4 py-2 font-semibold bg-white hover:bg-slate-50">Today</button>
              ...
              <div key={i} className="min-h-[130px] border border-slate-100 p-3 relative bg-white">
                ...
                <div className="mt-3 space-y-2">
                  {dayFlights.map((flight) => (
                    <button key={flight.id} onClick={() => onSelectFlight(flight)} className={`relative w-full rounded-xl border p-2 text-left text-white shadow-md transition hover:scale-[1.02] ${getAircraftClasses(flight.aircraft.color)} ${selectedFlightId === flight.id ? "ring-4 ring-amber-300" : ""}`}>
                      {flight.ownerOnboard && <div className="absolute -top-8 left-2 h-10 w-10 rounded-full bg-amber-400 shadow-lg flex items-center justify-center text-white text-2xl z-10">★</div>}
                      ...
                      {flight.ownerOnboard && <div className="mt-2 inline-flex items-center gap-1 rounded-full bg-white/20 px-2 py-1 text-xs font-bold">★ GGW</div>}
                    </button>
                  ))}
