...
import { passengers } from "../../data/library";
    ...
    <section className="rounded-3xl border border-slate-200 p-5">
      <div className="mb-4">
        ...
        <p className="text-sm text-slate-500">
          Select a favorite passenger, add them to the trip, or add someone new.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-3">
        ...
        >
          ...
          {passengers
            .filter((passenger) => passenger.name !== "GGW")
            .map((passenger) => (
        ...
        <button
          ...
          className="rounded-xl bg-[#0066D6] px-5 py-3 font-bold text-white flex items-center justify-center gap-2"
        >
          <Plus size={18} />
          Select / Add
        </button>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {selectedPassengers.map((passenger) => (
          <span
            key={passenger}
            className="inline-flex items-center gap-2 rounded-full bg-blue-50 px-4 py-2 text-sm font-bold text-[#0066D6]"
          >
            {passenger === "GGW" ? "★ GGW" : passenger}
            <button
              type="button"
              onClick={() => removePassenger(passenger)}
              className="rounded-full bg-white p-1 text-slate-500"
            >
              <X size={13} />
            </button>
          </span>
