...
import { Plane, CalendarDays, Clock, MapPin, Fuel, Hotel, Car, Utensils, Plus, Save } from "lucide-react";
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-[1fr_420px] gap-5">
      <section className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <Field label="Aircraft" icon={Plane}>
            <select className="input">{aircraft.map((item) => <option key={item.tail}>{item.tail} · {item.model}</option>)}</select>
          </Field>
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-5">
          <SavedSelect title="FBO" icon={Fuel} options={airportFbos.map((item) => `${item.name} ${item.airport}`)} />
          <SavedSelect title="Hotel" icon={Hotel} options={airportHotels.map((item) => item.name)} />
          <SavedSelect title="Rental Car" icon={Car} options={airportCars.map((item) => item.company)} />
          <SavedSelect title="Catering" icon={Utensils} options={airportCatering.map((item) => item.vendor)} />
        </div>

        <div className="mt-8">
          <label className="text-sm font-bold text-slate-700">Notes</label>
          <textarea className="input mt-2 min-h-[120px]" placeholder="Trip notes, passenger notes, release notes, special handling..." />
        </div>

        <div className="mt-8 flex flex-wrap gap-3">
          <button className="bg-[#0066D6] text-white rounded-xl px-6 py-3 font-bold flex items-center gap-2"><Save size={18} /> Save Trip</button>
          <button className="border border-slate-200 rounded-xl px-6 py-3 font-bold flex items-center gap-2"><Plus size={18} /> Add Leg</button>
      ...
      <aside className="space-y-5">
        <section className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
          <h2 className="text-xl font-bold text-slate-900">Passenger Logic</h2>
          <p className="text-sm text-slate-600 mt-3 leading-6">
            Everyone can be saved in the system, but only favorites should appear in the dropdown.
          </p>
        </section>
      </aside>
    </div>
...
function Field({ label, icon: Icon, children }: { label: string; icon: React.ElementType; children: React.ReactNode }) {
  return (
    <label>
      <div className="flex items-center gap-2 text-sm font-bold text-slate-700 mb-2"><Icon size={17} className="text-[#0066D6]" />{label}</div>
      {children}
    </label>
  );
}
  ...
  return (
    <div className="rounded-3xl border border-slate-200 p-5">
      <div className="flex items-center gap-3 mb-4">
        <div className="h-10 w-10 rounded-2xl bg-blue-50 flex items-center justify-center"><Icon size={20} className="text-[#0066D6]" /></div>
        <h3 className="font-bold text-slate-900">{title}</h3>
      </div>

      <select className="input">
        {options.length ? options.map((option) => <option key={option}>{option}</option>) : <option>No saved options for this airport</option>}
      </select>

      <button type="button" onClick={() => setShowManual(!showManual)} className="mt-3 text-sm font-bold text-[#0066D6]">+ Manually enter new {title}</button>

      {showManual && (
        <div className="mt-4 rounded-2xl bg-slate-50 p-4 space-y-3">
          <input className="input bg-white" placeholder={`${title} name`} />
          <input className="input bg-white" placeholder="Phone / contact" />
          <input className="input bg-white" placeholder="Address / notes" />
          <label className="flex items-center gap-2 text-sm font-bold text-slate-700">
            <input type="checkbox" defaultChecked />
            Save this {title} for future trips
          </label>
        </div>
      )}
    </div>
