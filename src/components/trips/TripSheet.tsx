...
export default function TripSheet() {
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-[1fr_360px] gap-5">
      <section id="trip-sheet" className="bg-white rounded-3xl border border-slate-200 shadow-sm p-8 print:shadow-none print:border-none print:rounded-none">
        <div className="flex flex-wrap items-start justify-between gap-5 border-b border-slate-200 pb-6 mb-6">
          <div>
            ...
            <h1 className="text-4xl font-bold text-slate-900">Flight Department Trip Sheet</h1>
            <p className="text-slate-500 mt-2">Operational trip summary for crew, scheduler, and dispatch review.</p>
          </div>
        ...
        <Section title="Crew & Passengers">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card icon={Users} title="Crew" lines={flight.crew} />
            <Card icon={Users} title="Passengers" lines={flight.passengers} />
          </div>
        </Section>

        <Section title="Trip Services">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return <section className="mb-7"><h2 className="text-xl font-bold text-slate-900 mb-4">{title}</h2>{children}</section>;
}
...
function Card({ icon: Icon, title, lines }: { icon: React.ElementType; title: string; lines: string[] }) {
  return <div className="rounded-2xl border border-slate-200 p-5"><div className="flex items-center gap-2 mb-3"><Icon size={19} className="text-[#0066D6]" /><h3 className="font-bold text-slate-900">{title}</h3></div><div className="space-y-2">{lines.map((line) => <p key={line} className="rounded-xl bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-700">{line}</p>)}</div></div>;
}
